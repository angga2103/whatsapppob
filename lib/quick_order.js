/**
 * Smart Natural Language Quick-Order Parser v2
 * Mendukung format pesan teks bebas pelanggan dengan urutan kata APA SAJA (Bebas Bolak-Balik).
 * Contoh valid:
 * - "pulsa tsel 10k 081234567890"
 * - "tsel pulsa 10k 081234567890"
 * - "pulsa 081234567890 5k"
 * - "5k 081234567890 pulsa"
 * - "data 081234567890 50k"
 * - "081234567890 data 50gb"
 * - "25gb data 081234567890"
 * - "token pln 50rb 141234567890"
 * - "141234567890 token 50k"
 * - "dana 20k 081234567890"
 * - "081234567890 dana 20k"
 * - "081234567890 10k" (auto infer Pulsa Telkomsel)
...
function parseQuickOrder(text, ppobCatalog = []) {
    ...
    // 3. Ekstraksi Denominasi / Nominal / Kuota
    ...
    if (kMatch) {
        ...
        if (rpMatch) {
            ...
            if (directMatch) {
                ...
                // E. Shorthand: jika ada pulsa/token dan angka tunggal 5, 10, 15, 20, 25, 50, 100
    ...
    // Default kategori ke Pulsa jika ada nomor HP + nominal tapi tidak menyebut 'data' atau kuota
    if (!category && targetType === 'phone' && nominal && brand && brand !== 'PLN') {
        category = 'Pulsa';
    }
    ...
    // KASUS 1: Hanya ada Nomor HP Tujuan (Belum pilih Pulsa / Data / Nominal)
    if (target && targetType === 'phone' && !nominal && !quotaVal && !category) {
        return {
            type: 'NEED_PRODUCT',
            matched: false,
            target,
            ...
            message: `💡 *Nomor Tujuan Terdeteksi:* \`${target}\` (${brand || 'Operator Seluler'})\n\n` +
                     ...
                     `Atau langsung ketik nominalnya, contoh:\n` +
                     `• \`pulsa ${target} 10k\`\n` +
                     `• \`data ${target} 25gb\``
        };
    }

    // KASUS 2: Produk & Nominal Terdeteksi, tapi Nomor Tujuan BELUM ADA
    if (!target && (nominal || quotaVal) && (category || brand)) {
        return {
            type: 'NEED_TARGET',
            matched: false,
            ...
            quotaVal,
            quotaUnit,
            message: `💡 *Pesanan Terdeteksi:* *${brand ? brand + ' ' : ''}${category || 'Produk'}${quotaVal ? ' ' + quotaVal + quotaUnit : (nominal ? ' ' + formatRupiah(nominal) : '')}*\n\n` +
    ...
    if (!target && !nominal && !quotaVal && isConversationalIntent) {
        return {
            type: 'GUIDE',
            matched: false,
            message: `🤖 *FORMAT ORDER CEPAT (BEBAS URUTAN):*\n\n` +
                     ...
                     `📶 *Paket Data:* \`data 081234567890 25gb\` atau \`081234567890 data 50gb\`\n` +
    ...
    // Jika tidak ada target atau tidak ada nominal/kuota, batalkan
    if (!target || (!nominal && !quotaVal)) return null;
    ...
    if (category === 'Data') {
        if (quotaVal) {
            // 1. Cari exact matching kuota GB: contoh "50 GB" atau "50GB"
            ...
            if (matchedQuota.length > 0) {
                ...
                // 2. Jika paket kuota exact tidak ditemukan (misal user ketik 99gb atau 55gb):
                // Buat saran paket data terdekat berdasarkan angka GB
            ...
            // 3. User mencari paket data berdasarkan budget rupiah (misal "data 50k")
