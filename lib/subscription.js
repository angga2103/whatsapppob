...
function getSubscriptions(filter = {}) {
    ...
    if (filter.buyer) {
        const normBuyer = normalizePhoneForCompare(filter.buyer);
        list = list.filter(s => {
            const sBuyer = normalizePhoneForCompare(s.buyer);
            const sPhone = normalizePhoneForCompare(s.buyerPhone);
            return (sBuyer && sBuyer.includes(normBuyer)) || (sPhone && sPhone.includes(normBuyer));
        });
    }
    if (filter.status) {
...
function getUserSubscriptions(buyerJid) {
    ...
    const normPhone = normalizePhoneForCompare(buyerJid);
    
    return list.filter(s => {
        const sJid = s.buyer ? (db.normalizeJid ? db.normalizeJid(s.buyer) : s.buyer) : '';
        const sNorm = normalizePhoneForCompare(s.buyerPhone || s.buyer);
        return (cleanJid && sJid === cleanJid) || (normPhone && sNorm === normPhone);
    });
}
